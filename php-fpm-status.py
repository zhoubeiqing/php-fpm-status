#!/usr/bin/python3 -u
# -*- coding: utf-8 -*-
import time,traceback,select,traceback,struct,socket,errno,types
# Need set configs start
# select a connect method and comment anthor one
socket_path='/var/run/php5-fpm.sock'
#host,port=('127.0.0.1',9000)
# Need set configs end
...
class PHPFPMCheck():
    ...
    def _load_page(self, url):
        """ load fastcgi page """
        try:
            #fcgi = fcgi_client.FCGIApp(host = '127.0.0.1',port = '9000')
            fcgi = self.app(self.socket,self.host,self.port)
            env = {
               #'SCRIPT_FILENAME': '/data/wwwroot/default/frank/index.php',
               'SCRIPT_FILENAME': url,
               'QUERY_STRING': '',
               'REQUEST_METHOD': 'GET',
               'SCRIPT_NAME': url,
            ...
            print('error: fastcgi load failed')
            exit()
            return '500', [], '', ''
    @property
    def ping(self):
        ...
        st = time.time()
        code, headers, out, err = self._load_page('/db8f91b4267')
        if not code.startswith('200') :
            print('ping: got response, but not correct')
            return 0            
...
class FCGIApp(object):
    ...
    def _getConnection(self):
        ...
        raise 'Launching and managing FastCGI programs not yet implemented'
